...
export default function GroupTrackingPage() {
  ...
  // Optionally, filter friends/leaderboard by search
  const filteredFriends = friends.filter(f => f.name.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className={`flex min-h-screen w-full overflow-hidden ${themeClasses.bg} transition-all duration-700`}>
      ...
      <main className="flex-1 px-4 md:px-12 py-8 md:py-12 overflow-y-auto">
        ...
        <section className="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4 animate-fadeInDown">
          <div>
            ...
            <p className={`text-lg ${themeClasses.muted} mt-2`}>Friend progress, leaderboard, and motivation—all in one dashboard.</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className={`flex items-center gap-1 px-5 py-2 rounded-xl font-semibold ${themeClasses.button} text-white ml-3 hover:scale-105 transition-all`}>
        ...
        {/* Leaderboard Section */}
        <section className={`mb-12 ${themeClasses.card} border rounded-3xl p-8 shadow-lg animate-fadeInUp`}>
            ...
            <table className="min-w-[380px] w-full text-left mt-2">
              ...
              <tbody>
                {[...leaderboard].sort(
                  ...
                  <tr key={entry.name} className="hover:bg-blue-500/10 transition rounded-xl font-medium">
                    <td className="py-2 pr-3 font-extrabold text-rose-400">{idx + 1}</td>
                    <td className="flex items-center gap-2 py-2">
                      <img src={entry.avatar} className="w-8 h-8 rounded-full border-2 border-blue-400" alt="avatar" />
                      <span>{entry.name}</span>
                      {idx === 0 && <Sparkles className="w-5 h-5 text-yellow-400 animate-bounce" title="Current Leader" />}
                    </td>
        ...
        <section className="mb-12 animate-fadeInUp grid md:grid-cols-2 xl:grid-cols-4 gap-8">
          {filteredFriends.length === 0 ? (
            <div className={`col-span-full text-xl font-semibold ${themeClasses.muted}`}>No friends found.</div>
          ) : (
            filteredFriends.map((f, idx) => (
              ...
              >
                <img src={f.avatar} alt={f.name} className="w-14 h-14 rounded-full border-2 border-blue-400 group-hover:scale-110 transition-transform" />
                <h4 className="text-xl font-bold mt-2 mb-1">{f.name}</h4>
                <div className="w-24 mb-1">
                  <div className="h-2 rounded-full bg-gray-200/30">
                    <div className={`h-2 rounded-full bg-gradient-to-r from-green-400 to-blue-500 animate-widthGrow`}
                ...
                <button className={`mt-2 px-4 py-2 rounded-xl ${themeClasses.button} text-white font-bold text-xs shadow hover:scale-105 transition-all`}>
                  View Progress
