...
import {
  Sun, Moon, Plus, Edit2, Trash2, ArrowRight, ListCheck, BarChart, User, Users, Mail
} from "lucide-react";
      ...
      <main className="flex-1 px-4 md:px-14 py-8 md:py-14 overflow-y-auto">
        <section className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 animate-fadeIn">
          ...
          <div className="flex items-center space-x-4">
            <div className={`${themeClasses.card} border rounded-xl p-6 flex flex-col items-center animate-pop`}>
              <span className="text-xs mb-1">Current Streak</span>
              <span className="text-2xl font-extrabold text-green-400">7 days</span>
            </div>
            <div className={`${themeClasses.card} border rounded-xl p-6 flex flex-col items-center animate-pop`} style={{ animationDelay: "100ms" }}>
          ...
          {routines.map((routine, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`px-3 py-1 rounded-lg bg-gradient-to-br from-blue-400 to-indigo-500 text-white text-xs font-bold animate-bounce`}>{routine.time}</span>
              </div>
              <ul className="mb-4">
                {routine.tasks.map((task, idx) => (
                  <li key={task.name} className="flex items-center mt-2">
                    <span className={`inline-block w-4 h-4 mr-2 rounded border-2 ${task.completed ? 'bg-green-400 border-green-400 animate-check' : 'border-gray-300'} transition`} />
                    <span className={`${task.completed ? 'line-through text-green-400' : themeClasses.muted}`}>{task.name}</span>
                  </li>
                ))}
              </ul>
              <div className="flex gap-3 items-center justify-between">
                <div className="flex-1 flex items-center">
                  ...
                  <span className="ml-2 text-xs font-bold">
                    {Math.round((routine.tasks.filter(t => t.completed).length / routine.tasks.length) * 100)}%
                  </span>
                </div>
                <div className="flex gap-2">
                  <Link to={`/routines/${routine.name.toLowerCase().replace(/\s+/g, '-')}`}>
                    <button className="rounded-full bg-blue-500/10 p-2 hover:bg-blue-500/20 transition animate-pop" title="Edit routine">
                      <Edit2 className="w-5 h-5 text-blue-400" />
                    </button>
                  </Link>
                  <button className="rounded-full bg-rose-500/10 p-2 hover:bg-rose-500/20 transition animate-pop" title="Delete routine" style={{ animationDelay: "100ms" }}>
              ...
              <div className="flex justify-between text-xs mt-2">
                ...
                <span className="text-indigo-400">Longest: {routine.longest} days</span>
              </div>
            </div>
          ))}
        </section>

        {/* Add Routine Modal with Animations */}
        {showAdd && (
          <div className="fixed z-50 inset-0 flex items-center justify-center bg-black/40 animate-fadeInFast">
            <div
              className={
                `w-full max-w-lg mx-auto border rounded-3xl p-8 shadow-2xl relative transition-all
            ...
            >
              ...
              <h2 className={`text-3xl font-extrabold text-center mb-7 ${isDark ? "text-blue-400" : "text-indigo-600"} animate-fadeInFast`}>
                Add New Routine
              </h2>
              <form className="space-y-6 animate-slideUp">
                <input className={`w-full p-3 rounded-xl border ...`} placeholder="Routine Name" />
                <div className="flex flex-col sm:flex-row gap-3">
                  <input className={`flex-1 p-3 rounded-xl border ...`} placeholder="Category (optional)" />
                  <input className={`flex-1 p-3 rounded-xl border ...`} placeholder="Tag (optional)" />
                </div>
                <div className="flex flex-col sm:flex-row gap-3">
                  <input className={`flex-1 p-3 rounded-xl border ...`} type="date" />
                  <input className={`flex-1 p-3 rounded-xl border ...`} type="date" placeholder="End Date" />
                </div>
                ...
                <input className={`w-full p-3 rounded-xl border ...`} type="time" placeholder="Reminder Time (optional)" />
                <textarea className={`w-full p-3 rounded-xl border ...`} rows={2} placeholder="Notes / Motivation (optional)" />
                <button
                  type="submit"
                  className={`w-full py-3 mt-2 rounded-xl font-bold text-lg shadow-lg transition-all hover:scale-105
                    ${isDark ? "bg-gradient-to-r from-blue-500 to-cyan-400" : "bg-gradient-to-r from-indigo-500 to-purple-500"} text-white animate-pop`}
                >
                  Create Routine
                </button>
              </form>
